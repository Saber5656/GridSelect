...
|---|---|---|---|---|---|
| Terminal | Terminal.app primary; iTerm2 optional; Warp tracked separately because GPU/custom rendering may expose different AX metadata. | `terminal-aligned-output` | Open a terminal with a monospace font. Ensure the window is wide enough and the fixture is not wrapped. | Run `cat tests/fixtures/rectangular-text/terminal-aligned-output/input.txt`, activate GridSelect, drag over rows `[2, 6)` and columns `[6, 24)`, then paste into a scratch text file and compare with `expected.txt`. | A supported terminal exposes visible text, range text, range bounds, and stable line geometry. The pasted result matches the fixture exactly. |
...
| Editor | TextEdit in plain-text mode with a monospace font; CotEditor/BBEdit optional native examples; VS Code/Cursor optional custom/Electron examples. | `editor-fixed-width-table` | Open `input.txt` in the editor, use a monospace font, and disable soft wrapping. | Select rows `[0, 5)` and columns `[17, 24)`, paste into a scratch text file, and compare with `expected.txt`. | Native text views are expected to expose AX text roles and range geometry. Custom editors may be partial or unsupported; record exact attributes when available. |
